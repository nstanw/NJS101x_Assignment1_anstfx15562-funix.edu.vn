...
const DiemDanh: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    if (values) {
      try {
        let phienLamViecHienTai = await phienLamViecService.addPhienLamViec(
          values.noiLam.label
        );
  ...
  const columnsTableListPhien = [
    ...
    },
    {
      ...
      render: (batDau: Date)=>{
        return (   
            <span>{new Date(batDau).toLocaleTimeString()}</span>
        );
      }
    },
    {
      ...
      render: (ketThuc: Date)=>{
        return (   
            <span>{new Date(ketThuc).toLocaleTimeString()}</span>
        );
      }
    },
    {
          ...
          onClick={async () => {
            try {
              ...
              phienLamViecs = phienLamViecs.filter(
                (d: any) =>
                  new Date(d.batDau).getDate() === new Date(Date.now()).getDate()
              );

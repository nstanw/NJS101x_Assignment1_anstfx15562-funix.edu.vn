            ...
            {isActive ? (
              <>
                <Descriptions title="Thông tin phiên làm việc">
                  ...
                  <Descriptions.Item label="Nơi làm">{phienHienTai.noiLam === "congTy" ? "Công Ty" : phienHienTai.noiLam === "home" ? "Nhà riêng" : "Khách hàng"}</Descriptions.Item>
                  <Descriptions.Item label="Thời gian bắt đầu">{new Date(phienHienTai.startTime).toLocaleTimeString()}</Descriptions.Item>
                  <Descriptions.Item label="Trạng thái">{phienHienTai.active ? "Đang làm" : "Không làm"}</Descriptions.Item>
                </Descriptions>
                <Button
                  type="primary"
                  danger
                  onClick={async () => {
                    try {
                      setIsLoading(true);
                      let listEndPhiens = await phienLamViecService.ketThucPhienLamViec();
                      setListEndPhiens(listEndPhiens);
                      setIsActive(false);
                      setIsLoading(false);
                    } catch (error) {}
                  }}
              ...
              <Row>
                <Col span={24}>
                  <Form form={form} onFinish={onFinish} initialValues={initialValues}>
                    ...
                    <Form.Item name="noiLam" label="Nơi làm việc" rules={[{ required: true }]}>
                      <Select onChange={handleChange} allowClear style={{ width: 200 }}>
                        <Option value="congTy" key="congTy">
                          Công Ty
                        </Option>
                ...
                {!isActive && (
                  <Col span={24}>
                    ...
                    <Table columns={columnsTableListPhien} dataSource={listEndPhiens} />
                  </Col>
                )}
              </Row>
            )}
